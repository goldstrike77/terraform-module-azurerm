...
data "azuread_user" "user" {
  for_each            = { for s in local.role_flat : format("%s%s%s%s%s", s.id, s.resource, s.type, s.name, s.role) => s if lower(s.type) == "user" }
  user_principal_name = each.value.name
}

# Query the object ID for each group.
data "azuread_group" "group" {
  for_each     = { for s in local.role_flat : format("%s%s%s%s%s", s.id, s.resource, s.type, s.name, s.role) => s if lower(s.type) == "group" }
  display_name = each.value.name
}

# Query the object ID for each service principal.
data "azuread_service_principal" "app" {
  for_each     = { for s in local.role_flat : format("%s%s%s%s%s", s.id, s.resource, s.type, s.name, s.role) => s if lower(s.type) == "app" }
  display_name = each.value.name
}
